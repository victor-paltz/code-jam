...
for i in range(NUM_TEST_CASES):
    ...
    for j in range(N):
        ...
        if q != -1:
            if q not in line:
                WrongAnswerExit(
                    p, i + 1, "Flavor {} was not liked by the customer!".format(q))
            if used[q]:
                WrongAnswerExit(
                    p, i + 1, "Flavor {} was already sold!".format(q))
            used[q] = True
            sold = sold + 1
    print("Case {}/{}: sold {} out of a possible {} lollipops.".format(i +
                                                                       1, NUM_TEST_CASES, sold, MAX[i]))
